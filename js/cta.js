/* cta.js — Floating WhatsApp button + scroll CTA bar — injected on every page */
(function(){
  ...
  style.textContent = `
    ...
    #wa-btn:hover {
      ...
      box-shadow:0 12px 36px rgba(37,211,102,.6),0 2px 8px rgba(0,0,0,.25);
      animation:none;
    }
    #wa-btn svg { width:28px;height:28px;fill:#fff; }
    @keyframes wa-pulse {
      0%,100%{box-shadow:0 6px 24px rgba(37,211,102,.45),0 0 0 0 rgba(37,211,102,.5);}
      50%{box-shadow:0 6px 24px rgba(37,211,102,.45),0 0 0 10px rgba(37,211,102,0);}
    }
    #wa-tooltip {
      background:rgba(8,14,28,.95);border:1px solid rgba(255,255,255,.1);
      border-radius:50px;padding:9px 16px;
      font-family:'Manrope',sans-serif;font-size:13px;font-weight:600;color:#fff;
    ...
    #wa-tooltip::after {
      ...
      border:6px solid transparent;border-left-color:rgba(8,14,28,.95);
      border-right:none;
    }
    ...
    #cta-bar {
      ...
      background:linear-gradient(90deg,#1a1f35,#0e1628);
    ...
    #cta-bar p {
      font-family:'Manrope',sans-serif;font-size:13.5px;font-weight:600;color:rgba(255,255,255,.7);
      margin:0;
    }
  ...
  waFloat.id = 'wa-float';
  waFloat.innerHTML = `
    <div id="wa-tooltip">Pulsuz Sınaq üçün yazın</div>
    <a id="wa-btn" href="${WA}" target="_blank" rel="noopener" aria-label="WhatsApp ilə əlaqə">
  ...
  ctaBar.id = 'cta-bar';
  ctaBar.innerHTML = `
    <p><strong>2 Həftəlik Pulsuz Sınaq</strong> — Kredit kartı tələb olunmur. İstənilən vaxt ləğv edin.</p>
    <div id="cta-bar-actions">
      <a href="${WA}" class="cb-cta" target="_blank" rel="noopener">Pulsuz Başla →</a>
      <button id="cta-bar-close" onclick="dismissBar()" aria-label="Bağla">✕</button>
    </div>
  `;
  document.body.appendChild(ctaBar);

  /* ── Show bar after 40% scroll ── */
  var barShown = false;
